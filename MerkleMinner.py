    ...
    while True:
        ...
        if temp_block['index'] <= chain_length:
            ...
            if temp_hash == sha256(str(temp_block).encode()).hexdigest():
                ...
                if temp_block['index'] <= tree.find().count():
                    print('---broadcast too late---')
                    continue
...
def listen_transaction(ip, port):
    ...
    while True:
        ...
        pre_transaction = conn.recv(10024).decode()
        print('----done---')
        print(data)
        ...
        if len(data.strip()) == 0:
            ...
            temp_tree = tree.find_one({'leaf':sha256(pre_transaction.encode()).hexdigest()})
            # need to be improved
            pre_transaction = json.loads(pre_transaction)
            if sum(rec_transaction['amount']) < pre_transaction['amount']:
                continue
            elif not temp_tree:
                continue
            else:
                temp_tree = Merkle2.Merkletree(all_hash=temp_tree)
                if not temp_tree.verify_tree(sha256(pre_transaction.encode()).hexdigest()):
                    continue
                else:
                    # two times verify
                    if not GenerateKey.verify_public_key(public_key=pub_key, address=rec_transaction['sour_address']):
                        continue
                    elif not GenerateKey.verify_sign(public_key=pub_key, sign=rec_transaction['signature'], data=(
                            str(rec_transaction['sour_address']) + str(rec_transaction['dest_address']) + str(
                            rec_transaction['amount']))):
                        print('---data---')
                        print((str(rec_transaction['sour_address']) + str(rec_transaction['dest_address']) + str(
                            rec_transaction['amount'])))
